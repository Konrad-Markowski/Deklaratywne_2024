%wyznaczania stopnia wielomianu
...
1) 
st(x)=1
st(c)=0, gdzie c - liczba stała
1) stopnie wielomianów złożonych określamy następująco:
st(-W)-st(W),
st(W1+W2)=st(W1-W2)=max(st(W1),st(W2)),
st(W1*W2)=st(W1)+st(W2),
st(W^N)=st(W)*N, gdzie N - liczba naturalna, N > 1
...
st(W^A,X,N):-integer(A), A>1, st(W,X,N1), N is N1*A.